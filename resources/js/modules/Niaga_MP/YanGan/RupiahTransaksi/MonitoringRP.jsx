...
export default function MonitoringRP() {
        ...
        const fetchData = async () => {
            ...
            try {
                const response = await fetch(
                    `http://127.0.0.1:8000/api/rupiah/monitoring`,
                );
                ...
                const actualData = data.tabelULP || [];
                ...
                if (actualData && actualData.length > 0) {
                    ...
                    const allAP = [
                        ...new Set(actualData.map((item) => item.unit_ap)),
                    ].filter(Boolean);
                    const allUP = [
                        ...new Set(actualData.map((item) => item.unit_up)),
                    ].filter(Boolean);
                    const allBulan = [
                        ...new Set(actualData.map((item) => item.bulan)),
                    ].filter(Boolean);
    ...
    const uniqueAP = useMemo(
        () =>
            [...new Set(tableData.map((item) => item.unit_ap))].filter(Boolean),
        [tableData],
    );
    const uniqueUP = useMemo(
        () =>
            [...new Set(tableData.map((item) => item.unit_up))].filter(Boolean),
        [tableData],
    );
    const uniqueBulan = useMemo(
        () => [...new Set(tableData.map((item) => item.bulan))].filter(Boolean),
        [tableData],
    );
    ...
    const filteredData = useMemo(() => {
        return tableData.filter((item) => {
            ...
            const matchAP =
                selectedAP.length === 0 || selectedAP.includes(item.unit_ap);
            const matchUP =
                selectedUP.length === 0 || selectedUP.includes(item.unit_up);
            const matchBulan =
                selectedBulan.length === 0 ||
                selectedBulan.includes(item.bulan);
                                ...
                                currentItems.map((row, idx) => (
                                    ...
                                    >
                                        ...
                                        {headers.map((headerKey, cellIdx) => (
                                            <td
                                                key={cellIdx}
                                                className="px-6 py-4 text-[#475569]"
                                            >
                                                {headerKey === "transaksi" &&
                                                !isNaN(row[headerKey])
                                                    ? Number(
